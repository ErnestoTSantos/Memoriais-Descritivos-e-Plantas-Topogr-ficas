...
class Project(models.Model):
    ...
    measurement_mode = models.CharField(
        max_length=20, choices=MEASUREMENT_MODE_CHOICES, default="planimetrico"
    )
    irradiation_origin_x = models.FloatField(null=True, blank=True)
    irradiation_origin_y = models.FloatField(null=True, blank=True)
    equipment_angular_error_seconds = models.FloatField(null=True, blank=True)
...
class ProcessRun(models.Model):
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="process_runs"
    )
    area_m2 = models.FloatField()
    perimeter_m = models.FloatField()
    closure_error_m = models.FloatField()
    planimetric_table = models.JSONField(default=dict, blank=True)
...
class Vertex(models.Model):
    process_run = models.ForeignKey(
        ProcessRun, on_delete=models.CASCADE, related_name="vertices"
    )
...
class Artifact(models.Model):
    process_run = models.ForeignKey(
        ProcessRun, on_delete=models.CASCADE, related_name="artifacts"
    )
